...
!macro _NSW_CenterWindow HWND HWPARENT
  ...
  System::Store S
  System::Alloc 16
  System::Call `user32::GetWindowRect(iR1,isR0)`
  ...
  IntOp $R3 $R3 - $R7
  IntOp $R4 $R4 - $R8
  System::Call `user32::SetWindowPos(iR2,i${HWND_TOP},iR3,iR4,i,i,i${SWP_NOSIZE})`
  System::Store L
  Pop $R2
  Pop $R1
!macroend
...
!macro _NSW_CreateWindow OUTVAR TITLE INTERVAL
  IsWindow ${OUTVAR} 0 +3
    ...
    Abort
  !ifndef __NSW_CURRENT
   !define __NSW_CURRENT 0
    !define nsWindows nsWindows
  !else
    !undef nsWindows
     !define nsWindows nsWindows${__NSW_CURRENT}
    !define /math __NSW_TEMPNUM ${__NSW_CURRENT} + 1
    !undef __NSW_CURRENT
    !define __NSW_CURRENT ${__NSW_TEMPNUM}
    !undef __NSW_TEMPNUM
  !endif
  ;
...
;
  ...
  Exch $R0
  System::Call "User32::DeleteMenu(i R0, i 5, i ${MF_BYPOSITION})"
  System::Call "User32::DeleteMenu(i R0, i 4, i ${MF_BYPOSITION})"
  System::Call "User32::DeleteMenu(i R0, i 3, i ${MF_BYPOSITION})"
  System::Call "User32::DeleteMenu(i R0, i 2, i ${MF_BYPOSITION})"
  System::Call "User32::DeleteMenu(i R0, i 0, i ${MF_BYPOSITION})"
  Exch $R0
!macroend
...
!macro _NSW_AddStyle CONTROL STYLE
  ...
  Push ${CONTROL}
  Exch $0
  System::Call "user32::GetWindowLong(i r0,i ${GWL_STYLE}) i.s"
  Exch $0
  System::Call "user32::SetWindowLong(i s,i ${GWL_STYLE},i $0|${STYLE})"
  Pop $0
...
!macro _NSW_AddExStyle CONTROL EXSTYLE
  ...
  Push ${CONTROL}
  Exch $0
  System::Call "user32::GetWindowLong(i r0,i ${GWL_EXSTYLE}) i.s"
  Exch $0
  System::Call "user32::SetWindowLong(i s,i ${GWL_EXSTYLE},i $0|${EXSTYLE})"
  Pop $0
...
!macro __NSW_GetWindowSize CONTROL Width Height

	Push $R9
	System::Call /NOUNLOAD "*(i, i, i, i) i.s"
	Pop $R9
  ;Get placement
	System::Call /NOUNLOAD 'User32::GetWindowRect(i, i) i (${CONTROL}, R9)'
	System::Call /NOUNLOAD "*$R9(i .s, i .s, i .s, i .s)"
	System::Free $R9
  ...
  IntOp ${Width} $2 - $0 ;width
  IntOp ${Height} $3 - $1 ;height

!macroend

!define NSW_GetWindowSize `!insertmacro __NSW_GetWindowSize`
...
!macro __NSW_GetWindowPos CONTROL Left Top
...
!define NSW_GetWindowPos `!insertmacro __NSW_GetWindowPos`
...
!macro __NSW_GetWindowPosEx CONTROL Left Top Width Height
  ...
  IntOp ${Width} $2 - ${left} ;width
  IntOp ${Height} $3 - ${top} ;height
  
!macroend

!define NSW_GetWindowPosEx `!insertmacro __NSW_GetWindowPosEx`
...
!macro __NSW_SetCtlText HWND FileName
  ...
  FileOpen $R2 $R1 r
  ${Unless} ${Errors}
    System::Call /NOUNLOAD "Kernel32::GetFileSize(i, i) i (R2, 0) .R3" 
    IntOp $R3 $R3 + 1
...
!macro __NSW_CB_GetSelection CONTROL VAR
  ...
  System::Call `user32::SendMessage(i ${CONTROL},i ${CB_GETCURSEL},i 0,i 0) i.s`
  System::Call `user32::SendMessage(i ${CONTROL},i ${CB_GETLBTEXT},i s,t .s)`
  Pop ${VAR}
...
System::Call `user32::SendMessage(i ${CONTROL},i ${LB_FINDSTRING},i -1,t "${STRING}") i.s`
System::Call `user32::SendMessage(i ${CONTROL},i ${LB_DELETESTRING},i s,i 0)`
...
!ifndef IID_IPicture
;  !define IID_IPicture {A3CCEDF7-2DE2-11D0-86F4-00A0C913F750}
